...
let AuthenticationService = class AuthenticationService {
    ...
    registerUser(email, phrase) {
        return this.ngFireAuth.createUserWithEmailAndPassword(email, phrase);
    }
    sendVerificationMail() {
        this.ngFireAuth.currentUser.then(user => user.sendEmailVerification())
            .then(() => {
            this.router.navigate(['verify-email']);
        });
    }
    phraseRecover(phraseResetEmail) {
        return this.ngFireAuth.sendPasswordResetEmail(phraseResetEmail)
            .then(() => {
    ...
    setUserData(user) {
        const userRef = this.afStore.doc(`users/${user.uuid}`);
        const userData = {
            uuid: user.uuid,
            email: user.email,
...
//# sourceMappingURL=default~pages-dashboard-dashboard-module~pages-login-login-module~pages-registration-registration-mo~f56288c2-es2015.js.map